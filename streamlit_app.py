...
with tab1:
    ...
    if uploaded_file is not None:
        ...
        if analyze_button and image is not None:
            with st.spinner("Analyse en cours..."):
                try:
                    ...
                    # Afficher les résultats dans col2
                    with col2:
        ...
        st.info("Choisissez une image pour commencer")
...
with tab3:
    st.subheader("Utilisez votre appareil photo")
    ...
    if camera_image is not None:
        ...
        if analyze_camera_button and image is not None:
            with st.spinner("Analyse en cours..."):
                try:
                    import uuid
                    prediction_id = str(uuid.uuid4())
                    ...
                    # Sauvegarder l'image
                    file_path = UPLOAD_DIR / f"{prediction_id}_camera.jpg"
                    image.save(file_path)
        ...
        elif analyze_camera_button and image is None:
            st.error("Erreur: impossible de charger la photo")
    else:
        st.info("Prenez une photo pour commencer")
